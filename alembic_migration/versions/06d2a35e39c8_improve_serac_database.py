...
def upgrade():
    ...
    op.add_column(
        'xreports', sa.Column('event_activity', new_activity_type), schema='guidebook'
    )
    op.add_column(
        'xreports_archives',
        ...
        op.execute(
            ...
            .where(
                xr.c.activities.contains(
                    sa.literal([old_value]).cast(old_activity_type)
                )
            )
    ...
    op.alter_column('xreports', 'event_activity', nullable=False, schema='guidebook')
    op.alter_column(
        'xreports_archives', 'event_activity', nullable=False, schema='guidebook'
    )
    ...
    op.add_column(
        'xreports', sa.Column('event_type_', new_event_type), schema='guidebook'
    )
    op.add_column(
        'xreports_archives',
    ...
    for old_value, new_value in type_conversions:
        op.execute(
            xr.update()
            .where(
                xr.c.event_type.contains(sa.literal([old_value]).cast(old_event_type))
            )
        ...
        op.execute(
            xra.update()
            .where(
                xra.c.event_type.contains(sa.literal([old_value]).cast(old_event_type))
            )
            .values(event_type_=op.inline_literal(new_value))
        )

    op.alter_column('xreports', 'event_type', nullable=False, schema='guidebook')
    op.alter_column(
        'xreports_archives', 'event_type', nullable=False, schema='guidebook'
    )
    ...
    op.alter_column(
        'xreports', 'event_type_', new_column_name='event_type', schema='guidebook'
    )
    op.alter_column(
        'xreports_archives',
        'event_type_',
    ...
    try:
        with open(
            ...
            for line in rr:
                print(
                    'update {} -> {}'.format(line[1], key_map[line[8].lower().encode()])
                )
                op.execute(
                    xr.update()
                    .where(xr.c.document_id == int(line[1]))
    ...
    new_autonomy_type = sa.Enum(
        'non_autonomous', 'autonomous', 'expert', name='autonomy_', schema='guidebook'
    )
    ...
    # Rename column
    op.alter_column(
        'xreports', 'autonomy_', new_column_name='autonomy', schema='guidebook'
    )
    op.alter_column(
        'xreports_archives', 'autonomy_', new_column_name='autonomy', schema='guidebook'
    )
    ...
    op.add_column(
        'xreports',
        sa.Column('qualification', qualification_type, nullable=True),
        schema='guidebook',
    )
    op.add_column(
        'xreports_archives',
...
def downgrade():
    ...
    old_autonomy_type = sa.Enum(
        'non_autonomous', 'autonomous', 'expert', name='autonomy', schema='guidebook'
    )
    new_autonomy_type = sa.Enum(
        'non_autonomous',
        'autonomous',
    ...
    op.alter_column('xreports', 'activities', nullable=False, schema='guidebook')
    op.alter_column(
        'xreports_archives', 'activities', nullable=False, schema='guidebook'
    )
