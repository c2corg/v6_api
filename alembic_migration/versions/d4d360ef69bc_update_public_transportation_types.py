...
from sqlalchemy import MetaData, Table, and_, any_, cast, func, text
...
def upgrade():
    ...
    # For waypoints having 'cable_car' in PublicTransportationTypes:
    ...
    op.execute(
        ...
        .where(
            and_(
                ...
                any_(waypoints.c.public_transportation_types)
                == op.inline_literal('cable_car'),
            )
        )
        .values({'lift_access': True})
    )
    op.execute(
        text(
            'UPDATE guidebook.waypoints'
            ' SET public_transportation_types ='
            ' array_remove('
            "   public_transportation_types::text[], 'cable_car'"
            ' )::guidebook.public_transportation_type[]'
            ' WHERE public_transportation_types IS NOT NULL'
            " AND 'cable_car' = ANY(public_transportation_types)"
        )
    )
    # Then fill the new public_transportation_types_ col with values
    # from PublicTransportationTypes. The intermediary string conversion
    ...
    op.execute(
        ...
        .values(
            {
                'public_transportation_types_': cast(
                    func.string_to_array(
                        waypoints.c.public_transportation_types_str, ','
                    ),
                    ArrayOfEnum(new_type),
                )
            }
        )
    )
    ...
    op.execute(
        ...
        .where(
            and_(
                ...
                any_(archives.c.public_transportation_types)
                == op.inline_literal('cable_car'),
            )
        )
        .values({'lift_access': True})
    ...
    op.execute(
        text(
            'UPDATE guidebook.waypoints_archives'
            ' SET public_transportation_types ='
            ' array_remove('
            "   public_transportation_types::text[], 'cable_car'"
            ' )::guidebook.public_transportation_type[]'
        ...
        .values(
            {
                'public_transportation_types_': cast(
                    func.string_to_array(
                        archives.c.public_transportation_types_str, ','
                    ),
                    ArrayOfEnum(new_type),
                )
            }
    ...
    op.drop_column(
        'waypoints_archives', 'public_transportation_types', schema='guidebook'
    )
    op.drop_column('waypoints', 'public_transportation_types_str', schema='guidebook')
    op.drop_column(
        'waypoints_archives', 'public_transportation_types_str', schema='guidebook'
    )
    drop_enum('public_transportation_type', schema='guidebook')

    # Rename enum
    op.execute(
        'ALTER TYPE guidebook.public_transportation_type_ RENAME TO public_transportation_type'
    )
