...
LangType = Literal[tuple(DefaultLangs)]  # type: ignore[valid-type]
...
def _python_type_for_column(col) -> type:
    ...
    if isinstance(col_type, (ArrayOfEnum, ARRAY)):
        ...
        if (
            ...
            and hasattr(col_type.impl, 'item_type')
        ):
            item = col_type.impl.item_type
        if item is not None and isinstance(item, Enum):
            return List[_literal_from_sa_enum(item)]  # type: ignore[valid-type]
...
def get_update_schema(
    document_schema: Type[BaseModel], *, name: str | None = None
) -> Type[BaseModel]:
    ...
    return create_model(schema_name, message=(str, ''), document=(document_schema, ...))
