...
class FieldSpec:
    ...
    def __init__(self, sa_model, columns, locale_fields=None, geometry_fields=None):
        self.sa_model = sa_model
        self.columns = list(columns)
        self.locale_fields = list(locale_fields) if locale_fields is not None else None
    ...
    def restrict(self, all_fields):
        ...
        *all_fields* uses the same dotted-prefix convention as the
        ...
        new_columns = [
            c for c in self.columns if c in fields or c in self._default_columns
        ]

        if self.locale_fields is not None:
            new_locale = [
                f
                for f in self.locale_fields
...
def build_field_spec(sa_model, includes, locale_fields=None, geometry_fields=None):
