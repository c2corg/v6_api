from sqlalchemy import Integer, and_, func, join, select, text, union
...
def _get_select_waypoints_for_routes_aggregated():
    ...
    return (
        select(
            ...
            func.array_agg(
                all_waypoints.c.waypoint_id, type_=postgresql.ARRAY(Integer)
            ).label('waypoint_ids'),
        )
...
def _get_select_waypoints_for_outings_aggregated():
    ...
    return (
        select(
            ...
            func.array_agg(
                waypoints_for_outings.c.waypoint_id, type_=postgresql.ARRAY(Integer)
            ).label('waypoint_ids'),
        )
...
def _get_select_users_for_outings_aggregated():
    ...
    return (
        select(
            Association.child_document_id.label('outing_id'),
            func.array_agg(
                Association.parent_document_id, type_=postgresql.ARRAY(Integer)
            ).label('user_ids'),
        )
...
def _get_select_users_for_routes_aggregated():
    ...
    return (
        select(
            ...
            func.array_agg(DocumentTag.user_id, type_=postgresql.ARRAY(Integer)).label(
                'user_ids'
            ),
        )
