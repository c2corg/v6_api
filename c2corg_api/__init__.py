...
def process_new_waypoint(mapper, connection, geometry):
    ...
    for place in places_data["places_nearby"]:
        ...
        existing_stop_id = connection.execute(
            existing_stop_query, {"stop_id": stop_id}).scalar()
...
def calculate_route_duration(mapper, connection, route):
    ...
    climbing_activities = [
        'rock_climbing', 'ice_climbing', 'mountain_climbing',
    ...
    if is_climbing and getattr(
        ...
        if (route.route_length is None or height_diff_up is None) and route.height_diff_difficulties > 0:
            ...
            log.warn(
                f"Route {route_id}: Database updated with calculated_duration = {calculated_duration_in_days} days (based on difficulties_height only).")
            return

    # Convertir les valeurs de base (avec COALESCE pour remplacer NULL par 0)
    h = float(
        route.route_length if route.route_length is not None else 0) / 1000  # km
    ...
    for activity in activities:
        ...
        if activity in climbing_activities:
            ...
            if diff_height is not None and diff_height > 0:
                ...
                if dp > 0 and d_diff > dp:
                    log.warn(
                        f"Route {route_id}: Inconsistent difficulties_height ({d_diff}m) > height_diff_up ({dp}m). Skipping for this activity.")
            ...
            log.warn(
                f"Calculated climbing route duration for route {route_id} (activity {activity}): {
                    dm:.2f} hours")
        ...
        # CALCUL POUR LES AUTRES ITINÉRAIRES (NON GRIMPANTS)
            ...
            log.warn(
                f"Calculated standard route duration for route {route_id} (activity {activity}): {
                    dm:.2f} hours")
    ...
    if min_duration is None or min_duration < min_duration_hours or min_duration > max_duration_hours:
        log.warn(
            f"Route {route_id}: Calculated duration ({
                min_duration:.2f} hours) is out of bounds (min={min_duration_hours}h, max={max_duration_hours}h) or NULL. Setting duration to NULL.")
        calculated_duration_in_days = None
    else:
    ...
    """), {"duration": calculated_duration_in_days, "route_id": route_id})
    log.warn(
        f"Route {route_id}: Database updated with calculated_duration = {calculated_duration_in_days} days.")
