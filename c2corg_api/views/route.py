...
from sqlalchemy import case, column, func, literal_column, or_, text, union
...
from c2corg_api.models.common.attributes import Activities, PublicTransportationRatings
...
from c2corg_api.schemas.route import CreateRouteSchema, UpdateRouteSchema
...
validate_route_create = make_validator_create(fields_route, 'activities', Activities)
validate_route_update = make_validator_update(fields_route, 'activities', Activities)
...
def validate_main_waypoint(is_on_create, request, **kwargs):
    ...
    association.
    """
    doc = request.validated if is_on_create else request.validated.get('document', {})
...
@resource(collection_path='/routes', path='/routes/{id}', cors_policy=cors_policy)
class RouteRest(DocumentRest):
    ...
    def get(self):
        return self._get(
            ...
            adapt_schema=route_schema_adaptor,
            include_maps=True,
    ...
    def collection_post(self):
        linked_waypoints = self.request.validated.get('associations', {}).get(
            'waypoints', []
        )
...
@resource(path='/routes/update_public_transportation_rating', cors_policy=cors_policy)
class RoutePublicTransportationRatingRest(ACLDefault):
    ...
    def get(self):
        waypoint_extrapolation = (
            self.request.params.get('waypoint_extrapolation') or True
        )
...
class ReachableRouteRest(DocumentRest):
    ...
    def get(self):
        ...
        query, count = build_reachable_route_query(self.request.GET, meta_params)
        ...
        if query is None:
            ...
            results = (
                query.limit(meta_params['limit']).offset(meta_params['offset']).all()
            )
        ...
        areas_objects = (
            DBSession.query(Area).filter(Area.document_id.in_(areas_id)).all()
        )
        ...
        for route, areas in results:
            ...
            for area in areas:
                ...
                if area_obj:
                    json_areas.append(to_json_dict(area_obj, schema_listing_area))
...
def _get_default_geom_from_main_wp(route):
    return (
        DBSession.query(DocumentGeometry.geom)
...
def update_all_pt_rating(waypoint_extrapolation=True):
    ...
    parent_routes = (
        DBSession.query(Association.parent_document_id.label('route_id'))
    ...
    children_routes = (
        DBSession.query(Association.child_document_id.label('route_id'))
...
def _pt_rating(starting_waypoints, ending_waypoints, route_types):
    ...
    """
    # Enum members are ordered best-to-worst; build a rank for comparison.
    _rank = {r.value: i for i, r in enumerate(PublicTransportationRatings)}
    default_rating = 'unknown service'
    worst_rating = 'no service'

    def best(a, b):
        """Return the better (lower rank) of two ratings."""
        return a if _rank[a] <= _rank[b] else b

    def worst(a, b):
        """Return the worse (higher rank) of two ratings."""
        return a if _rank[a] >= _rank[b] else b
...
def build_reachable_route_query(params, meta_params):
    ...
    """
    all_filtered_routes_reachable_ids, total_hits = get_all_filtered_docs(
    ...
    ordering_case = case(
        {doc_id: idx for idx, doc_id in enumerate(all_filtered_routes_reachable_ids)},
    ...
    query = (
        ...
        .filter(Route.document_id.in_(all_filtered_routes_reachable_ids))
        .join(
            Association,
            or_(
...
def get_routes_reachable_ids():
    ...
    # extract their ids
    all_routes_reachable_ids = set([r.document_id for r in all_routes_reachable])
