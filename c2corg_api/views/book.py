...
from c2corg_api.models.book import BOOK_TYPE, ArchiveBook, Book, schema_book
from c2corg_api.models.common.fields_book import fields_book
from c2corg_api.models.document import ArchiveDocumentLocale
from c2corg_api.schemas.book import CreateBookSchema, UpdateBookSchema
...
validate_associations_create = functools.partial(validate_associations, BOOK_TYPE, True)
validate_associations_update = functools.partial(
    validate_associations, BOOK_TYPE, False
)


@resource(collection_path='/books', path='/books/{id}', cors_policy=cors_policy)
class BookRest(DocumentRest):
    ...
    def get(self):
        return self._get(book_documents_config, schema_book, include_areas=False)

    @restricted_json_view(
        validators=[
            make_pydantic_validator(CreateBookSchema),
    ...
    @restricted_json_view(
        validators=[
            make_pydantic_validator(UpdateBookSchema),
            validate_id,
...
@resource(path='/books/{id}/{lang}/{version_id}', cors_policy=cors_policy)
