    ...
    *field_prefix* is an optional string prepended to error field names
    ...
    if geometry:
        for field in _GEOM_FIELDS:
            ...
            try:
                    ...
                    if geom_type not in check_types:
                        if request is not None:
                            request.errors.add(
                                ...
                                'Got: %s.' % (check_types, geo.get('type', '').upper()),
                            )
                        continue
                ...
                if request is not None:
                    request.errors.add(
                        'body', _field_path(field), 'Invalid geometry: %s' % value
                    )
    ...
    if doc:
        _convert_geojson_to_wkb(
            doc, srid, request=request, allowed_geometry_types=allowed_geometry_types
        )
    ...
    for key, value in data.items():
        ...
        if isinstance(value, dict) and field_info is not None:
            ...
            if isinstance(inner_model, type) and issubclass(inner_model, BaseModel):
                ...
                value = _strip_defaults_and_collections(value, nested_fs, inner_model)
        result[key] = value
    return result
    return result
...
def make_pydantic_validator(
    pydantic_model, allowed_geometry_types=None, strip_defaults=True
):
    ...
    def pydantic_body_validator(request, **kwargs):
        ...
        try:
            ...
            for error in exc.errors():
                ...
                # Paths that drill into nested *objects* (e.g.
                # ('areas', 0, 'document_id')) are left intact.
                ...
                # Avoid duplicate errors for the same collapsed location
                if location in seen_locations:
                    continue
                seen_locations.add(location)

                msg = error['msg']
                # Pydantic v2 prepends "Value error, " for ValueError-based
                # validation errors.  Strip it for cleaner API responses.
                if msg.startswith('Value error, '):
                    msg = msg[len('Value error, ') :]
                # Normalise "Field required" → "Required" to match the
                # legacy cornice/colander convention used across the API.
                if msg == 'Field required':
                    msg = 'Required'
                request.errors.add('body', location, msg)
        ...
        if strip_defaults:
            ...
            dumped = _strip_defaults_and_collections(
                validated.model_dump(), nested_fields_set, pydantic_model
            )
        else:
        ...
        _convert_geojson_to_wkb(
            dumped, request=request, allowed_geometry_types=allowed_geometry_types
        )
