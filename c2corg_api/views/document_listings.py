...
from c2corg_api.views import set_best_locale, to_json_dict
...
def _get_documents_from_ids(
    ...
    return [
        to_json_dict(
            doc,
            documents_config.schema
            if not documents_config.adapt_schema
            else documents_config.adapt_schema(documents_config.schema, doc),
...
def _set_areas_for_documents(documents, lang, db: Session | None = None):
    ...
    for document in documents:
        ...
        document.areas = [to_json_dict(m, schema_listing_area) for m in document._areas]
