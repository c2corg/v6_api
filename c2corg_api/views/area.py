...
from c2corg_api.models.area import AREA_TYPE, ArchiveArea, Area, schema_area
...
from c2corg_api.models.document import ArchiveDocumentLocale, UpdateType
from c2corg_api.schemas.area import CreateAreaSchema, UpdateAreaSchema
...
validate_associations_create = functools.partial(validate_associations, AREA_TYPE, True)
validate_associations_update = functools.partial(
    validate_associations, AREA_TYPE, False
)


@resource(collection_path='/areas', path='/areas/{id}', cors_policy=cors_policy)
class AreaRest(DocumentRest):
    ...
    def get(self):
        return self._get(area_documents_config, schema_area, include_areas=False)

    @restricted_json_view(
        validators=[
            make_pydantic_validator(
                CreateAreaSchema, allowed_geometry_types=['POLYGON', 'MULTIPOLYGON']
            ),
    ...
    def collection_post(self):
        return self._collection_post(schema_area, after_add=insert_associations)

    @restricted_json_view(
        validators=[
            make_pydantic_validator(
                UpdateAreaSchema, allowed_geometry_types=['POLYGON', 'MULTIPOLYGON']
            ),
            validate_id,
