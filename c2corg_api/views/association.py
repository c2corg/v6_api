...
def validate_association(request, **kwargs):
    ...
    if parent_document_type and child_document_type:
        ...
        if association_type not in valid_associations:
            request.errors.add('body', 'association', 'invalid association type')
        else:
            validate_association_permission(
                request,
                parent_document_id,
...
@resource(
    collection_path='/associations', path='/associations/{id}', cors_policy=cors_policy
)
class AssociationRest(ACLDefault):
    @restricted_json_view(
        validators=[make_pydantic_validator(SchemaAssociation), validate_association]
    )
    def collection_post(self):
        ...
        association.parent_document_type = self.request.validated[
            'parent_document_type'
        ]
        association.child_document_type = self.request.validated['child_document_type']

        if exists_already(association):
            raise HTTPBadRequest('association (or its back-link) exists already')

        DBSession.add(association)
        DBSession.add(association.get_log(self.request.authenticated_userid))
    ...
    @restricted_json_view(validators=[make_pydantic_validator(SchemaAssociation)])
    def collection_delete(self):
        ...
        log = association.get_log(self.request.authenticated_userid, is_creation=False)
...
def _check_required_associations(association):
    if _is_main_waypoint_association(association):
        raise HTTPBadRequest(
            'as the main waypoint of the route, this waypoint can not be disassociated'
        )
    elif _is_last_waypoint_of_route(association):
        raise HTTPBadRequest(
            'as the last waypoint of the route, this waypoint can not be disassociated'
        )
    elif _is_last_route_of_outing(association):
        raise HTTPBadRequest(
            'as the last route of the outing, this route can not be disassociated'
        )
