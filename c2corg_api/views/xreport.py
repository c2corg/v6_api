...
from c2corg_api.schemas.xreport import CreateXreportSchema, UpdateXreportSchema
from c2corg_api.views import cors_policy, restricted_json_view, set_private_cache_header
...
@resource(collection_path='/xreports', path='/xreports/{id}', cors_policy=cors_policy)
class XreportRest(DocumentRest):
    ...
    @restricted_json_view(
        validators=[
            make_pydantic_validator(
                CreateXreportSchema, allowed_geometry_types=['POINT']
            ),
    ...
    @restricted_json_view(
        validators=[
            make_pydantic_validator(
                UpdateXreportSchema, allowed_geometry_types=['POINT']
            ),
            validate_id,
...
def _has_permission(request, xreport_id):
    ...
    information of the xreport or to change the xreport. Only moderators
    and users that are currently assigned to the xreport can modify it.
    """
