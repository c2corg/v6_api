...
def load_feed(doc_ids, limit, user_id=None):
    if not doc_ids:
        ...
        doc_changes = DBSession.query(DocumentVersion) \
            .options(load_only(DocumentVersion.lang),
                     joinedload(DocumentVersion.history_metadata).load_only(
                        HistoryMetaData.id,
                        HistoryMetaData.user_id,
                        HistoryMetaData.comment,
                        HistoryMetaData.written_at).
                     joinedload(HistoryMetaData.user).load_only(
                        User.id,
                        User.name,
                        User.username,
                        User.lang)) \
            .options(joinedload(DocumentVersion.document).load_only(
                    Document.version,
                    Document.document_id,
                    Document.type,
                    Document.quality)) \
            .options(joinedload(
                    DocumentVersion.document_locales_archive).load_only(
