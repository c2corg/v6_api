...
class UserPreferredLanguageRest(ACLDefault):
    @restricted_json_view(
        renderer='json', validators=[make_pydantic_validator(UpdatePreferredLangSchema)]
    )
    def post(self):
...
def validate_account_fields(request, **kwargs):
    ...
    if currentpassword is not None and len(currentpassword) < 3:
        request.errors.add('body', 'currentpassword', 'Shorter than minimum length 3')
    ...
    if forum_username is not None:
        if not is_unused_user_attribute(
            ...
            request.errors.add('body', 'forum_username', 'Already used forum name')

    newpassword = validated.get('newpassword')
    if newpassword is not None and len(newpassword) < 3:
        request.errors.add('body', 'newpassword', 'Shorter than minimum length 3')
...
class UserAccountRest(ACLDefault):
    ...
    @restricted_json_view(
        renderer='json',
        validators=[
            make_pydantic_validator(UpdateAccountSchema),
    ...
    def post(self):
        ...
        if 'email' in validated and validated['email'] != user.email:
            ...
            user.update_validation_nonce(Purpose.change_email, VALIDATION_EXPIRE_DAYS)
