...
from c2corg_api.models.topo_map import MAP_TYPE, TopoMap, schema_topo_map
from c2corg_api.models.topo_map_association import update_map
from c2corg_api.schemas.topo_map import CreateTopoMapSchema, UpdateTopoMapSchema
...
class TopoMapRest(DocumentRest):
    ...
    @restricted_json_view(
        validators=[
            make_pydantic_validator(
                CreateTopoMapSchema, allowed_geometry_types=['POLYGON', 'MULTIPOLYGON']
            ),
            validate_map_create,
        ],
    ...
    def collection_post(self):
        return self._collection_post(schema_topo_map, after_add=insert_associations)

    @restricted_json_view(
        validators=[
            make_pydantic_validator(
                UpdateTopoMapSchema, allowed_geometry_types=['POLYGON', 'MULTIPOLYGON']
            ),
            validate_id,
            validate_map_update,
        ],
    ...
    def put(self):
        return self._put(TopoMap, schema_topo_map, after_update=update_associations)
