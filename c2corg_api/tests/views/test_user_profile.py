...
class TestUserProfileRest(BaseDocumentTestRest):
    ...
    def test_get_collection_paginated(self):
        self.assertResultsEqual(
            self.get_collection({'offset': 0, 'limit': 0}, user='contributor'), [], 8
        )

        self.assertResultsEqual(
            self.get_collection({'offset': 0, 'limit': 1}, user='contributor'),
    ...
    def test_get_unauthenticated_private_profile(self):
        ...
        response = self.app.get(
            self._prefix + '/' + str(self.profile1.document_id), status=200
        )
        body = response.json
    ...
    def test_get_unconfirmed_user(self):
        ...
        self.app.get(
            self._prefix + '/' + str(self.profile3.document_id),
            headers=headers,
            status=404,
        )
    ...
    def test_put_wrong_user(self):
        ...
        self.app_put_json(
            self._prefix + '/' + str(self.profile1.document_id),
            body,
            headers=headers,
            status=403,
        )
    ...
    def test_put_success_all(self):
        ...
        (body, profile) = self.put_success_all(
            body, self.profile1, user='moderator', check_es=False, cache_version=3
        )
    ...
    def _check_es_index(self):
        ...
        search_doc = SearchUser.get(
            id=self.profile1.document_id, index=elasticsearch_config['index']
        )
