...
from c2corg_api.security.pyramid_jwt_policy import IntegerSubJWTAuthenticationPolicy
...
class TestIntegerSubJWTAuthenticationPolicy(unittest.TestCase):
    def setUp(self):
        ...
        self.policy = IntegerSubJWTAuthenticationPolicy(
            private_key=self.secret, algorithm='HS256', auth_type='JWT'
        )
    ...
    def test_get_claims_legacy_format(self):
        ...
        request = self._make_request('JWT token="' + self.token + '"')
...
class TestExtractToken:
    ...
    def test_extract_token_legacy_format(self):
        ...
        assert extract_token(request) == 'my.jwt.token'
