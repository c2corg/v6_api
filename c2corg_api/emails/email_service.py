...
class EmailLocalizator(object):
    ...
    def get_translation(self, lang, key):
        ...
        try:
            ...
            log.exception('The %s translation for %s could not be read' % (lang, key))
            return self._get_file_content('fr', key)


class EmailService:
    instance: Optional['EmailService'] = None
    ...
    def _send_email(self, to_address: str, subject: str, body: str):
        log.debug('Sending email to %s through %s' % (to_address, self.mail_server))
        ...
        if asbool(self.settings.get('mail.ssl', False)):
            smtp = smtplib.SMTP_SSL(
                self.settings['mail.host'], port=self.settings['mail.port']
            )
        else:
            smtp = smtplib.SMTP(
                self.settings['mail.host'], port=self.settings['mail.port']
        ...
        if self.settings.get('mail.username', '') not in ('', 'None'):
            smtp.login(self.settings['mail.username'], self.settings['mail.password'])
    ...
    def send_request_change_password(self, user, link):
        body = self._(user.lang, 'password_change_body') % (link, user.username)
        self._send_email(
            user.email, subject=self._(user.lang, 'password_change_subject'), body=body
        )
    ...
    def send_rate_limiting_alert(self, user):
        ...
        if user.blocked:
            body = self._('fr', 'rate_limiting_blocked_alert_body') % (user.name, url)
        else:
            body = self._('fr', 'rate_limiting_alert_body') % (
                user.name,
                url,
                user.ratelimit_times,
            )
        self._send_email(
...
def get_email_service_from_settings(settings):
    """Return the ``EmailService`` singleton, creating it from *settings*
    if it doesn't exist yet.

    Unlike ``get_email_service`` this does not require a Pyramid request
    and can be used from FastAPI code.
    """
    if not EmailService.instance:
        EmailService.instance = EmailService(settings)
    return EmailService.instance
