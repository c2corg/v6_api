...
from c2corg_api.search.mapping_types import (
    QEnum,
    QEnumArray,
    QEnumRange,
    QInteger,
...
class SearchRoute(SearchDocument):
    ...
    activities = QEnumArray('act', model_field=Route.activities)
    elevation_min = QInteger('rmina', range=True)
    elevation_max = QInteger('rmaxa', range=True)
    height_diff_up = QInteger('hdif', range=True)
    height_diff_down = QInteger('ddif', range=True)
    route_length = QInteger('rlen', range=True)
    difficulties_height = QInteger('ralt', range=True)
    height_diff_access = QInteger('rappr', range=True)
    height_diff_difficulties = QInteger('dhei', range=True)
    route_types = QEnumArray('rtyp', model_field=Route.route_types)
    orientations = QEnumArray('fac', model_field=Route.orientations)
    durations = QEnumRange(
        'time', model_field=Route.durations, enum_mapper=sortable_route_duration_types
    )
    glacier_gear = QEnum('glac', model_field=Route.glacier_gear)
    configuration = QEnumArray('conf', model_field=Route.configuration)
    ski_rating = QEnumRange(
        'trat', model_field=Route.ski_rating, enum_mapper=sortable_ski_ratings
    )
    ski_exposition = QEnumRange(
        'sexpo',
        model_field=Route.ski_exposition,
        enum_mapper=sortable_exposition_ratings,
    )
    labande_ski_rating = QEnumRange(
        'srat',
        model_field=Route.labande_ski_rating,
        enum_mapper=sortable_labande_ski_ratings,
    )
    labande_global_rating = QEnumRange(
        'lrat',
        model_field=Route.labande_global_rating,
        enum_mapper=sortable_global_ratings,
    )
    global_rating = QEnumRange(
        'grat', model_field=Route.global_rating, enum_mapper=sortable_global_ratings
    )
    engagement_rating = QEnumRange(
        'erat',
        model_field=Route.engagement_rating,
        enum_mapper=sortable_engagement_ratings,
    )
    risk_rating = QEnumRange(
        'orrat', model_field=Route.risk_rating, enum_mapper=sortable_risk_ratings
    )
    ...
    ice_rating = QEnumRange(
        'irat', model_field=Route.ice_rating, enum_mapper=sortable_ice_ratings
    )
    mixed_rating = QEnumRange(
        'mrat', model_field=Route.mixed_rating, enum_mapper=sortable_mixed_ratings
    )
    exposition_rock_rating = QEnumRange(
        'rexpo',
        model_field=Route.exposition_rock_rating,
        enum_mapper=sortable_exposition_rock_ratings,
    )
    rock_free_rating = QEnumRange(
        'frat',
        model_field=Route.rock_free_rating,
        enum_mapper=sortable_climbing_ratings,
    )
    rock_required_rating = QEnumRange(
        'rrat',
        model_field=Route.rock_required_rating,
        enum_mapper=sortable_climbing_ratings,
    )
    aid_rating = QEnumRange(
        'arat', model_field=Route.aid_rating, enum_mapper=sortable_aid_ratings
    )
    via_ferrata_rating = QEnumRange(
        'krat',
        model_field=Route.via_ferrata_rating,
    ...
    hiking_mtb_exposition = QEnumRange(
        'hexpo',
        model_field=Route.hiking_mtb_exposition,
        enum_mapper=sortable_exposition_ratings,
    )
    snowshoe_rating = QEnumRange(
        'wrat', model_field=Route.snowshoe_rating, enum_mapper=sortable_snowshoe_ratings
    )
    mtb_up_rating = QEnumRange(
        'mbur', model_field=Route.mtb_up_rating, enum_mapper=sortable_mtb_up_ratings
    )
    mtb_down_rating = QEnumRange(
        'mbdr', model_field=Route.mtb_down_rating, enum_mapper=sortable_mtb_down_ratings
    )
    mtb_length_asphalt = QInteger('mbroad', range=True)
    mtb_length_trail = QInteger('mbtrack', range=True)
    mtb_height_diff_portages = QInteger('mbpush', range=True)
    rock_types = QEnumArray('rock', model_field=Route.rock_types)
    climbing_outdoor_type = QEnumArray('crtyp', model_field=Route.climbing_outdoor_type)
    slackline_type = QEnum('sltyp', model_field=Route.slackline_type)
    ...
    def to_search_document(document, index):
        ...
        SearchDocument.copy_enum_range_fields(
            search_document, document, SearchRoute.ENUM_RANGE_FIELDS, SearchRoute
        )

        for locale in document.locales:
            search_document['title_' + locale.lang] = get_title(
                locale.title, locale.title_prefix
            )
        ...
        if document.associated_users_ids:
            ...
            search_document['users'] = document.associated_users_ids.user_ids
