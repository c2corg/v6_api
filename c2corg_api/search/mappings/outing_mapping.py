...
class SearchOuting(SearchDocument):
    ...
    activities = QEnumArray('act', model_field=Outing.activities)
    frequentation = QEnumRange(
        'ofreq',
        model_field=Outing.frequentation,
        enum_mapper=sortable_frequentation_types,
    )
    elevation_max = QInteger('oalt', range=True)
    height_diff_up = QInteger('odif', range=True)
    length_total = QInteger('olen', range=True)
    public_transport = QBoolean('owpt', is_bool=True)
    elevation_access = QInteger('oparka', range=True)
    elevation_up_snow = QInteger('swlu', range=True)
    elevation_down_snow = QInteger('swld', range=True)
    condition_rating = QEnumRange(
        'ocond',
        model_field=Outing.condition_rating,
        enum_mapper=sortable_condition_ratings,
    )
    snow_quantity = QEnumRange(
        'swquan',
        model_field=Outing.snow_quantity,
        enum_mapper=sortable_snow_quantity_ratings,
    )
    snow_quality = QEnumRange(
        'swqual',
        model_field=Outing.snow_quality,
        enum_mapper=sortable_snow_quality_ratings,
    )
    glacier_rating = QEnumRange(
        'oglac', model_field=Outing.glacier_rating, enum_mapper=sortable_glacier_ratings
    )
    avalanche_signs = QEnumArray('avdate', model_field=Outing.avalanche_signs)
    ski_rating = QEnumRange(
        'trat', model_field=Outing.ski_rating, enum_mapper=sortable_ski_ratings
    )
    labande_global_rating = QEnumRange(
        'lrat',
        model_field=Outing.labande_global_rating,
        enum_mapper=sortable_global_ratings,
    )
    global_rating = QEnumRange(
        'grat', model_field=Outing.global_rating, enum_mapper=sortable_global_ratings
    )
    height_diff_difficulties = QInteger('dhei', range=True)
    engagement_rating = QEnumRange(
        'erat',
        model_field=Outing.engagement_rating,
        enum_mapper=sortable_engagement_ratings,
    )
    equipment_rating = QEnumRange(
        'prat',
        model_field=Outing.equipment_rating,
        enum_mapper=sortable_equipment_ratings,
    )
    ice_rating = QEnumRange(
        'irat', model_field=Outing.ice_rating, enum_mapper=sortable_ice_ratings
    )
    rock_free_rating = QEnumRange(
        'frat',
        model_field=Outing.rock_free_rating,
        enum_mapper=sortable_climbing_ratings,
    )
    via_ferrata_rating = QEnumRange(
        'krat',
        model_field=Outing.via_ferrata_rating,
        enum_mapper=sortable_via_ferrata_ratings,
    )
    hiking_rating = QEnumRange(
        'hrat', model_field=Outing.hiking_rating, enum_mapper=sortable_hiking_ratings
    )
    snowshoe_rating = QEnumRange(
        'wrat',
        model_field=Outing.snowshoe_rating,
        enum_mapper=sortable_snowshoe_ratings,
    )
    mtb_up_rating = QEnumRange(
        'mbur', model_field=Outing.mtb_up_rating, enum_mapper=sortable_mtb_up_ratings
    )
    mtb_down_rating = QEnumRange(
        'mbdr',
        model_field=Outing.mtb_down_rating,
    ...
    def to_search_document(document, index):
        ...
        SearchDocument.copy_fields(search_document, document, SearchOuting.FIELDS)

        SearchDocument.copy_enum_range_fields(
            search_document, document, SearchOuting.ENUM_RANGE_FIELDS, SearchOuting
        )
        ...
        if document.associated_routes_ids:
            ...
            search_document['routes'] = document.associated_routes_ids.route_ids

        return search_document


SearchOuting.queryable_fields = QueryableMixin.get_queryable_fields(SearchOuting)
SearchOuting.queryable_fields['date'] = QDateRange('date', 'date_start', 'date_end')
SearchOuting.queryable_fields['period'] = QPeriod('period', 'date_start', 'date_end')
