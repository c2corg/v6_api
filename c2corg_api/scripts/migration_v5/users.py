...
from c2corg_api.scripts.migration_v5.migrate_base import MigrateBase, parse_php_object
...
class MigrateUsers(MigrateBase):
    ...
    def migrate(self):
        ...
        query = text(
            'select id, login_name, topo_name, email, '
        ...
        def get_group_by_id(gid):
            results = self.connection_source.execute(
                text(
                    'select user_id from app_users_groups where group_id = ' + str(gid)
                )
            ).fetchall()
        ...
        with transaction.manager, batch:
            ...
            for user_in in self.connection_source.execute(query):
                ...
                filter_areas = self.get_areas(preferences.get('filtered_places'))
        ...
        all_area_ids = set(
            area_id for (area_id,) in self.session_target.query(Area.document_id).all()
        )
        ...
        with transaction.manager, batch_filter_area:
            for user_id, area_id in filter_areas_for_users:
                ...
                if area_id in all_area_ids:
                    batch_filter_area.add(dict(user_id=user_id, area_id=area_id))
            zope.sqlalchemy.mark_changed(self.session_target)
    ...
    def get_areas(self, places_raw):
        ...
        return [int(id_raw) for id_raw in area_ids_raw]
