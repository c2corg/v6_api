...
from c2corg_api.models.document import ArchiveDocumentGeometry, DocumentGeometry
...
from c2corg_api.scripts.migration_v5.batch import SimpleBatch
from c2corg_api.scripts.migration_v5.documents.batch_document import DocumentBatch
from c2corg_api.scripts.migration_v5.migrate_base import MigrateBase
...
class CreateClimbingSiteRoutes(MigrateBase):
    ...
    def migrate(self):
        ...
        with transaction.manager:
            ...
            links_for_climbing_site = self._create_dummy_routes(climbing_sites)
    ...
    def _create_dummy_routes(self, climbing_sites):
        ...
        with route_locale_batch:
            ...
            for route_locale_archive in route_locale_archives:
                route_locale_batch.add_archive_documents([route_locale_archive])
    ...
    def _create_routes(self, climbing_site_ids, route_batch):
        ...
        for climbing_site_id, outings_ids, elevation in climbing_site_ids:
            (
                route_id,
                route_archive_id,
                route_geometry_archive_id,
                route_locale_id,
                route_locale_archive_id,
                history_id,
                version_id,
            ) = routes_ids.get_ids()
            ...
            # default geometries
            route_geometry = dict(document_id=route_id, version=1)
    ...
    def _create_links_for_routes(self, links_for_climbing_site):
        ...
        with association_batch, association_log_batch:
            for climbing_site_id in links_for_climbing_site:
                route_id, outings_ids = links_for_climbing_site[climbing_site_id]
...
class RouteIds(object):
    def __init__(self, session_target):
        self.current_route_id = get_last_id(session_target, 'documents', 'document_id')
        ...
        self.current_history_id = get_last_id(session_target, 'history_metadata', 'id')
