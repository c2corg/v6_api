...
from c2corg_api.scripts.migration_v5.documents.versions import tables_union
...
class MigrateAssociations(MigrateBase):
    ...
    def _migrate(self, label, query_count, query, model, get_entity):
        ...
        with transaction.manager, batch:
            ...
            for entity_in in self.connection_source.execute(query):
                ...
                if entity_in.main_id not in [
                    224228,
                    436917,
                ] and entity_in.linked_id not in [224228, 436917]:
    ...
    def _remove_duplicate_associations(self):
        ...
        with transaction.manager:
            self.session_target.execute(text(SQL_DELETE_DUPLICATE_ASSOCIATONS_LOG))
            self.session_target.execute(text(SQL_DELETE_DUPLICATE_ASSOCIATONS))
            zope.sqlalchemy.mark_changed(self.session_target)
        print('Done')
