...
class MigrateUserProfiles(MigrateDocuments):
    ...
    def migrate(self):
        ...
        with transaction.manager:
            last_locale_id = self.connection_source.execute(
                text(
                    'select max(document_i18n_archive_id) '
                    'from app_documents_i18n_archives;'
                )
            ).fetchone()[0]
            last_archive_id = self.connection_source.execute(
                text('select max(document_archive_id) from app_documents_archives;')
            ).fetchone()[0]
            ...
            for row in profileless_users:
                ...
                locale = DocumentLocale(
                    id=last_locale_id, document_id=user_id, lang='fr', title=''
                )
    ...
    def get_document(self, document_in, version):
        categories = (
            [document_in.category] if document_in.category is not None else None
        )
        return dict(
            document_id=document_in.id,
            ...
            categories=self.convert_types(
                categories, MigrateUserProfiles.user_categories, skip_values=[0, 2, 5]
            ),
        )
    ...
    user_categories = {'1': 'amateur', '3': 'club', '4': 'institution'}
