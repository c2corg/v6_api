...
from c2corg_api.scripts.migration_v5.migrate_base import parse_php_object
...
class MigrateOutings(MigrateDocuments):
    ...
    def get_query(self):
        return (
            ...
            '   ST_Simplify(ST_SetSRID(oa.geom, 3857), 5) geom, '
    ...
    def get_document(self, document_in, version):
        ...
        return dict(
            ...
            access_condition=self.convert_type(
                document_in.access_status, MigrateOutings.access_conditions
            ),
            avalanche_signs=self.convert_types(
                document_in.avalanche_date, MigrateOutings.avalanche_signs
            ),
            condition_rating=self.convert_type(
                document_in.conditions_status, MigrateOutings.condition_ratings
            ),
            ...
            frequentation=self.convert_type(
                document_in.frequentation_status, MigrateOutings.frequentation_types
            ),
            glacier_rating=self.convert_type(
                document_in.glacier_status, MigrateOutings.glacier_ratings
            ),
            height_diff_down=document_in.height_diff_down,
            height_diff_up=document_in.height_diff_up,
            hut_status=self.convert_type(
                document_in.hut_status, MigrateOutings.hut_status
            ),
            length_total=document_in.outing_length,
            lift_status=self.convert_type(
                document_in.lift_status, MigrateOutings.lift_status
            ),
    ...
    def get_document_locale(self, document_in, version):
        ...
        return dict(
            ...
            conditions_levels=php_to_json(
                document_in.conditions_levels, document_in.document_i18n_archive_id
            ),
    ...
    access_conditions = {'2': 'cleared', '4': 'snowy', '6': 'closed_snow'}
