...
tables_union = (
    ' union '.join(['select id, redirects_to from ' + t for t in tables])
    ...
    def migrate(self):
        ...
        with transaction.manager:
            ...
            last_metadata_id = self.connection_source.execute(
                text('select max(history_metadata_id) from app_history_metadata;')
            ).fetchone()[0]
            last_version_id = self.connection_source.execute(
                text('select max(documents_versions_id) from app_documents_versions;')
            ).fetchone()[0]
            ...
            for row in profileless_users:
                ...
                archive = (
                    self.session_target.query(ArchiveDocument)
                    .filter(ArchiveDocument.document_id == user_id)
                    .one()
                )
                locale = (
                    self.session_target.query(ArchiveDocumentLocale)
                    .filter(ArchiveDocumentLocale.document_id == user_id)
                    .one()
                )

                meta_data = HistoryMetaData(
                    id=last_metadata_id, comment='creation', user_id=user_id
                )
