...
DEFAULT_QUALITY = QualityTypes.medium
...
class MigrateDocuments(MigrateBase):
    ...
    def _migrate(self, locales=False):
        self.start('{0}{1}'.format('locales of ' if locales else '', self.get_name()))

        query_count = (
            self.get_count_query_locales() if locales else self.get_count_query()
        )
            ...
            for document_in in self.connection_source.execute(query):
                ...
                if current_document_id is None:
                    ...
                    if current_document_id != document_in.id:
                        print(
                            'WARNING: no latest version for {0}'.format(
                                current_document_id
                            )
                        )
                        archives = []
                ...
                if locales:
                    ...
                    document_archive = self.get_document_archive(document_in, version)
                ...
                if document_in.is_latest_version:
                    if locales:
                        ...
                        document = self.get_document_locale(document_in, version)
                    else:
                        document = self.get_document(document_in, version)
                        geometry = self.get_document_geometry(document_in, version)
                        # do not migrate any empty geom for non geom types
    ...
    summary_regex = re.compile(r'(\[abs(tract)?\]){1}?([\s\S]*?)(\[/abs(tract)?\]){1}?')
    ...
    wikilink_profiles_regex = re.compile(r'\[\[(users)([^|]*)\|([^\]]+)\]\]')  # noqa
