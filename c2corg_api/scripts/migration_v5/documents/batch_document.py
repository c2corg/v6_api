...
class DocumentBatch(Batch):
    ...
    def __init__(
        ...
        batch_size,
        model_document,
        model_archive_document,
        model_geometry,
        model_archive_geometry,
    ):
    ...
    def flush(self):
        if self.documents:
            self.session.bulk_insert_mappings(self.model_document, self.documents)
            self.session.flush()
            self.documents = []
        if self.archive_documents:
            ...
            self.archive_documents = []
        if self.geometries:
            self.session.bulk_insert_mappings(self.model_geometry, self.geometries)
            self.session.flush()
            self.geometries = []
