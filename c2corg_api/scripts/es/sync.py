...
from sqlalchemy import and_, literal, or_, union
from sqlalchemy.orm import joinedload

from c2corg_api.models import document_locale_types, document_types, es_sync
...
def sync_es(session, batch_size=1000):
    ...
    if not last_update:
        raise Exception(
            'No last update time, run `fill_index` to do an '
...
def get_changed_documents(session, last_update):
    ...
    return (
        session.query(Document.document_id, Document.type)
        .join(DocumentVersion, Document.document_id == DocumentVersion.document_id)
        .join(
            HistoryMetaData, HistoryMetaData.id == DocumentVersion.history_metadata_id
        )
...
def get_changed_users(session, last_update):
    ...
    """
    return (
        session.query(User.id, literal(USERPROFILE_TYPE))
...
def get_changed_routes_wr(session, last_update):
    ...
    """
    return (
        session.query(AssociationLog.child_document_id, literal(ROUTE_TYPE))
        .filter(
            and_(
...
def get_changed_routes_and_outings_ww(session, last_update):
    ...
    """
    select_changed_waypoints = (
        session.query(AssociationLog.child_document_id.label('waypoint_id'))
        .filter(
            and_(
                AssociationLog.parent_document_type == WAYPOINT_TYPE,
    ...
    select_changed_waypoint_children = (
        session.query(Association.child_document_id.label('waypoint_id'))
    ...
    select_all_changed_waypoints = union(
        select_changed_waypoints.select(), select_changed_waypoint_children.select()
    ).cte('all_changed_waypoints')

    select_changed_routes = (
        session.query(Association.child_document_id.label('route_id'))
    ...
    select_changed_outings = (
        session.query(Association.child_document_id.label('outing_id'))
        .select_from(select_changed_routes)
        .join(
            Association,
            and_(
                Association.parent_document_id == select_changed_routes.c.route_id,
...
def get_deleted_locale_documents(session, last_update):
    ...
    """
    return (
        session.query(ESDeletedLocale.document_id, ESDeletedLocale.type)
...
def get_deleted_documents(session, last_update):
    ...
    return (
        session.query(ESDeletedDocument.document_id, ESDeletedDocument.type)
...
def get_tagged_documents(session, last_update):
    ...
    return (
        session.query(DocumentTagLog.document_id, DocumentTagLog.document_type)
...
def sync_documents(session, changed_documents, batch_size):
    ...
    with batch:
        ...
        for doc_type, document_ids in docs_per_type.items():
            if document_ids:
                docs = get_documents(session, doc_type, batch_size, document_ids)
...
def add_routes_for_waypoints(session, docs_per_type):
    ...
    linked_route_ids = [
        rid
        for (rid,) in session.query(Route.document_id)
        .filter(Route.main_waypoint_id.in_(changed_waypoint_ids))
        .all()
    ]
...
def get_documents(
    session, doc_type, batch_size, document_ids=None, ignore_redirects=False
):
    ...
    base_query = base_query.options(
        joinedload(clazz.locales.of_type(locales_clazz)).load_only(*locale_fields)
    ).options(joinedload(clazz.geometry).load_only(DocumentGeometry.lon_lat))

    if clazz != Area:
        base_query = base_query.options(
            joinedload(clazz._areas).load_only(Area.document_id)
        )

    if clazz == Route:
        base_query = base_query.options(
            joinedload(Route.associated_waypoints_ids).load_only(
    ...
    if clazz == Outing:
        base_query = base_query.options(
            joinedload(Outing.associated_waypoints_ids).load_only(
